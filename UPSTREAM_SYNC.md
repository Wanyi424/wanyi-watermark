...
| `douyin-video/scripts/douyin_downloader.py`（CLI，硅基流动 + 大文件分段，仅抖音） | `wanyi_watermark/cli.py` + `transcription.py`（已多平台化 + 双后端：dashscope / siliconflow） | 已回迁硅基流动后端与大文件分段。 |
...
> 多数延后；其中 3.1/3.2/3.3 已落地。每项标注：上游来源、价值、前置验证、代码内 TODO 位置。

### 3.1 硅基流动 SenseVoice 作为可选 ASR 后端 —— ✅ 已落地
- **状态变更**：已实现。CLI `--backend siliconflow` 启用，环境变量 `SILICONFLOW_API_KEY`。
- **已实现**：`wanyi_watermark/transcription.py` → `_transcribe_siliconflow()`，按域名补 Referer/UA 下载视频，ffmpeg 提取音频，调用硅基流动 `FunAudioLLM/SenseVoiceSmall` API。
- **上游来源**：`douyin-video/scripts/douyin_downloader.py` → `transcribe_single_audio()`

### 3.2 大文件自动分段转写 —— ✅ 已落地
- **状态变更**：已实现。集成在 SiliconFlow 后端中（>1h 或 >50MB 自动按 9min/段切割）。
- **已实现**：`wanyi_watermark/transcription.py` → `_split_audio()` / `_extract_text_from_audio()`。
- **说明**：仅 SiliconFlow 后端需要分段；dashscope 百炼 URL 直传为服务端异步转写，原生支持长音频，无需客户端分段。
...
| `ffmpeg-python` 已有使用场景 | SiliconFlow 后端的音频提取/分段依赖 ffmpeg；dashscope 链路仍无需 | `transcription.py` / `pyproject.toml` |
