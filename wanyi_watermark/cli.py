...
2. 下载无水印视频 / 图集
3. 提取视频文案（支持阿里云百炼 / 硅基流动 SenseVoice 双后端）
...
def cmd_extract(link: str, output: Optional[str], save_video: bool, model: Optional[str], backend: Optional[str] = None) -> int:
    from .resolver import resolve_media
    ...
    effective_backend = (backend or os.getenv("ASR_BACKEND", "dashscope")).lower()
    print(f"正在从视频中提取文案（后端: {effective_backend}）...")
    text = transcribe_video_url(data["url"], model=model, backend=backend, show_progress=True)
...
def main() -> None:
    ...
    parser.add_argument("--model", "-m", help="语音识别模型 (可选，默认按后端自动选择)")
    parser.add_argument("--backend", "-b", choices=["dashscope", "siliconflow"],
                        help="ASR 后端: dashscope(百炼URL直传) / siliconflow(硅基流动SenseVoice)；"
                             "缺省读 ASR_BACKEND 环境变量，再缺省 dashscope")
