...
1. 解析链接得到无水印视频直链
2. 按 ASR_BACKEND 选择后端:
   - **dashscope（默认）**: 调用阿里云百炼 paraformer-v2 (URL 直传, 无需本地下载)
   - **siliconflow**: 下载视频 → ffmpeg 提取音频 → 大文件自动分段(>1h/50MB) → 调用硅基流动 SenseVoice
...
- 检查对应的环境变量是否已设置且有效（`DASHSCOPE_API_KEY` 或 `SILICONFLOW_API_KEY`）
- 使用硅基流动后端时需确保本地已安装 ffmpeg
